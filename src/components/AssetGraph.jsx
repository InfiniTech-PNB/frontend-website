...
const AssetGraph = ({
    ...
    const resizeCanvas = () => {
      ...
      if (
        ...
        transformRef.current.offsetX =
          rect.width / 2 - (rect.width / 2) * transformRef.current.zoom;
        transformRef.current.offsetY =
          rect.height / 2 - (rect.height / 2) * transformRef.current.zoom;
      }
    };
  ...
  const getNodeStyle = (node) => {
    const isSelected =
      !node.isVirtual &&
      !node.isRoot &&
      selectedAssets.includes(node.asset?._id);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      linksRef.current.forEach((link) => {
        ...
        const flowSpeed = 0.8;
        const particlePos =
          (time * flowSpeed + (source.x + source.y) * 0.005) % 1.0;
  ...
  return (
    <div
      ref={containerRef}
      style={{ height: "420px" }}
      className="relative w-full bg-slate-950 dark:bg-slate-950/70 border border-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl"
    >
      {/* Canvas Element */}
      <canvas
        ref={canvasRef}
